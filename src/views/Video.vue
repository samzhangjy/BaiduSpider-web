        ...
        <div :key="resultId">
          <div class="grid">
              <bd-card
                ...
                :key="index"
                class="col_md-4 col_lg-3 col_sm-6 col_xs-12"
                :title="result.title"
                :url="result.url"
                :cover="result.img"
...
export default {
  ...
  methods: {
    getResults: async function () {
      this.loading = true
      await SearchService.searchVideo(this.query, this.curPage, window.location.href).then((data) => {
        if (data == 'Error') {
          this.error = true
          this.loading = false
          return
    ...
    getPages: function () {
      for (var i = 1; i <= this.total; i++) {
        this.pages.push({
          url: `/search/video?q=${encodeURIComponent(this.query)}&pn=${i}`,
          pn: i
        })
      }
    }
