<template>
  <div>
    ...
    <bd-container style="margin: 10px 50px 10px 50px" class="grid" v-if="!loading && !error">
        ...
        <div :key="resultId">
            ...
            <div v-for="(result, index) in resultsNews" :key="index">
              <h4>
                ...
                <span v-if="result.time">
                  <small class="text-secondary">&nbsp;&nbsp;{{ result.time }}</small>
                </span>
              </h4>
            ...
            <div v-if="resultsVideo">
              <div class="grid">
                <bd-card
                  ...
                  :key="index"
                  :class="`${resultsExtra ? 'col_md-6 col_lg-4 col_sm-12 col_xs-12' : 'col_md-4 col_lg-3 col_sm-6 col_xs-12'}`"
                  :title="result.title"
                  :url="result.url"
                  :cover="result.cover"
          ...
          <div v-if="resultsRelated">
            ...
            >
              ...
              >
                <router-link :to="`/search/web?q=${encodeURIComponent(result)}&pn=1`">
                  {{ result }}
                </router-link>
              </span>
            </bd-card>
      ...
      <div class="col-4 extra" v-if="resultsExtra">
        <bd-card
          ...
          :coverType="`${resultsBaike['cover-type'] ? resultsBaike['cover-type'] : 'none'}`"
        >
        </bd-card>
        <bd-card
          v-if="resultsTieba"
          ...
          coverType="image"
        >
          <span class="text-secondary">关注人数：{{ resultsTieba.followers }}</span>
          <span class="text-secondary">&nbsp;&nbsp;累计发帖：{{ resultsTieba.total }}</span>
          <br /><br />
        ...
        >
          <div>
            <h4>简介</h4>
            <p v-if="resultsGitee.license">开源协议：{{ resultsGitee.license }}</p>
            <p v-if="resultsGitee.lang">开发语言：{{ resultsGitee.lang }}</p>
            <span>
              Star：{{ resultsGitee.star }}&nbsp;&nbsp;
            </span>
            <span>
              Fork：{{ resultsGitee.fork }}&nbsp;&nbsp;
            </span>
            <span>
              Watch：{{ resultsGitee.watch }}
            </span>
            <br />
            <h4>码云指数</h4>
            <img :src="resultsGitee.status" style="width: 100%" class="bd-img-need-dark" />
          </div>
          <br />
        </bd-card>
      </div>
    ...
    getResults: async function () {
      ...
      await SearchService.searchWeb(this.query, this.curPage, window.location.href).then((data) => {
        ...
        for (var i = 0; i < this.results.results.length; i++) {
          ...
          } else if (this.results.results[i].type === 'tieba') {
            this.resultsTieba = this.results.results[i].result
            this.resultsExtra = true
          } else if (this.results.results[i].type === 'blog') {
            this.resultsBlog = this.results.results[i].result
            this.resultsExtra = true
          } else if (this.results.results[i].type === 'gitee') {
            this.resultsGitee = this.results.results[i].result
            this.resultsExtra = true
          } else if (this.results.results[i].type === 'related') {
            this.resultsRelated = this.results.results[i].results
          }
        }
        this.resultId += 1
        this.loading = false
